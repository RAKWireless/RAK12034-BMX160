...
void DFRobot_BMX160::setLowPower()
{
  ...
  writeBmxReg(BMX160_ACCEL_CONFIG_ADDR, 0x94);
  delay(100);

  // writeBmxReg(BMX160_PMU_TRIGGER_ADDR, 0x48);
  // delay(10);

  writeBmxReg(BMX160_COMMAND_REG_ADDR, 0x11); // 0x12-low power  0x11-normal (accel)
  // delay(65);
  delayMicroseconds(3500);
  writeBmxReg(BMX160_COMMAND_REG_ADDR, 0x16); // 0x15-normal  0x16-suspend   0x17-Fast Start-Up(Gyro)

  delay(70);
  writeBmxReg(BMX160_COMMAND_REG_ADDR, 0x1b); // 0x1a-low power ,0x1b-suspend ,0x19-normal(Magn)
...
void DFRobot_BMX160::wakeUp()
{
  ...
  setMagnConf();
  delay(100);
  writeBmxReg(BMX160_ACCEL_CONFIG_ADDR, 0x28);
  delay(100);
  writeBmxReg(BMX160_COMMAND_REG_ADDR, 0x11);
  delayMicroseconds(3500);
  /* Set gyro to normal mode */
